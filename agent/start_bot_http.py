...
class SimpleRateLimiter:
    ...
    def acquire(cls):
        with cls._lock:
            now = time.time()
            wait = max(0, cls._min_interval - (now - cls._last_call))
            if wait > 0:
                time.sleep(wait)
