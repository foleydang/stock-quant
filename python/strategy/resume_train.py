...
用法:
  python strategy/resume_train.py --model daily
  python strategy/resume_train.py --model 30m
"""
...
# Optuna 搜索时用的固定参数（与 train.py objective 函数一致）
FIXED_PARAMS = {
...
def main():
    ...
    parser.add_argument('--params', type=str, default='',
                        help='JSON字符串, 覆盖 best_params.json')
    parser.add_argument('--cvs', type=str, default='',
                        help='CV结果JSON: {"spearman":0.38,"rmse":0.044,"mae":0.033}')
    parser.add_argument('--n-estimators', type=int, default=0,
                        help='覆盖 n_estimators (默认 2000)')
    ...
    # n_estimators: 最终模型用更多树，搜索时只用了 500
    n_estimators = args.n_estimators or best_params.get('n_estimators', 0) or 2000
    ...
    print(f" n_estimators={n_estimators}, objective={best_params['objective']}")
    ...
    print(f"数据: {len(X):,}条, {X.shape[1]}特征, 目标: mean={y.mean():.4f}, std={y.std():.4f}")
    ...
    if rm:
        ...
        print(f"  去冗余: {sum(keep)}/{sum(keep)+len(rm)}")
    else:
        print(f"  无高相关特征 (|r|>0.95)")

    # --- SelectFromModel (用足够数据 + 合理early stopping) ---
    print("\n🔧 SelectFromModel 特征选择...")
    split = int(len(X) * 0.8)
    X_train, X_val = X[:split][:-5000], X[:split][-5000:]
    y_train, y_val = y[:split][:-5000], y[:split][-5000:]

    # 选择用 500 棵树就够了
    sel_params = {**best_params, 'n_estimators': 500}
    sel = lgb.LGBMRegressor(**sel_params)
    sel.fit(X_train, y_train,
            eval_set=[(X_val, y_val)],
            callbacks=[lgb.early_stopping(50, verbose=False)])
    n_trees = sel.best_iteration_ or sel_params['n_estimators']
    print(f"  选择模型训练了 {n_trees} 棵树")
    ...
    # --- 最终模型 ---
    print(f"\n🏋️ 训练最终模型 ({n_estimators} 棵树, {len(feature_names)} 特征)...")
    final_model = lgb.LGBMRegressor(**best_params)
    final_model.fit(X, y)
    actual_trees = final_model.best_iteration_ or n_estimators

    imp = final_model.feature_importances_
    top = np.argsort(imp)[-20:][::-1]
    print(f"\n  实际训练了 {actual_trees} 棵树")
    print(f"  Top 20 特征:")
    for idx in top:
        print(f"    {feature_names[idx]}: {imp[idx]:.0f}")

    # 快速自检: 预测值是否有区分度
    pred_sample = final_model.predict(X[:1000])
    print(f"  预测值范围: [{pred_sample.min():.4f}, {pred_sample.max():.4f}], "
          f"std={pred_sample.std():.6f}")
    ...
    size_mb = os.path.getsize(os.path.join(d, 'model.pkl')) / 1024 / 1024
    ...
    print(f"\n✅ 模型已保存: {d}/model.pkl ({size_mb:.1f} MB)")
    print(f"  特征数: {len(feature_names)}, 树数: {actual_trees}")
    if cfg['cv_spearman']:
        print(f"  CV Rank IC: {cfg['cv_spearman']:.4f}")
