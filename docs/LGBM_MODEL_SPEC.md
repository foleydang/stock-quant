...
基于 LightGBM 构建双层量化模型，用于 A 股择时选股。日线模型负责选股（α层），30分钟模型负责择时（γ层），通过 Bagging Ensemble 提升稳定性。
...
| 参数 | 值 | 说明 |
|------|----|------|
| objective | multiclass (3分类) | 涨(0) / 震荡(1) / 跌(2) |
...
```
收益率 = (future_close - current_close) / current_close
...
# 4. 模型产出
models/lgb_daily/model.pkl  (~125-200MB)
models/lgb_30m/model.pkl   (~125-200MB)
...
models/
├── lgb_daily/
│   ├── model.pkl       # 5模型ensemble (125-200MB)
│   └── meta.json       # 训练元信息
└── lgb_30m/
    ├── model.pkl       # 5模型ensemble (125-200MB)
    └── meta.json       # 训练元信息
...
```python
{
    'models': [LGBMClassifier × 5],  # 5个独立模型
    'feature_names': [...],          # 特征名列表
